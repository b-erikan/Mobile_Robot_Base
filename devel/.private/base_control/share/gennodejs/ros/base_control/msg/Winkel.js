// Auto-generated. Do not edit!

// (in-package base_control.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class Winkel {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.rotate = null;
    }
    else {
      if (initObj.hasOwnProperty('rotate')) {
        this.rotate = initObj.rotate
      }
      else {
        this.rotate = 0.0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type Winkel
    // Serialize message field [rotate]
    bufferOffset = _serializer.float32(obj.rotate, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type Winkel
    let len;
    let data = new Winkel(null);
    // Deserialize message field [rotate]
    data.rotate = _deserializer.float32(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 4;
  }

  static datatype() {
    // Returns string type for a message object
    return 'base_control/Winkel';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return 'be12cf1d559af9e1d7c33b2780105139';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    float32 rotate
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new Winkel(null);
    if (msg.rotate !== undefined) {
      resolved.rotate = msg.rotate;
    }
    else {
      resolved.rotate = 0.0
    }

    return resolved;
    }
};

module.exports = Winkel;
